...
<body>
  ...
  <main id="login-view" class="login-view is-hidden" aria-labelledby="login-title">
    <section class="login-card">
      ...
      <h1 id="login-title">欢迎回到 Nanocount</h1>
      <p class="subtle">输入管理员密码，查看属于你的网站访问数据。</p>
      <form id="login-form">
        ...
        <div class="password-wrap">
          ...
          <button id="toggle-password" class="text-button" type="button" aria-label="显示密码">显示</button>
        </div>
        <p id="login-error" class="form-error" role="alert"></p>
        <button class="button button-primary button-wide" type="submit">安全登录 <span aria-hidden="true">→</span></button>
      </form>
      <p class="privacy-note"><span aria-hidden="true">◆</span> 密码只用于创建同源、HttpOnly 的签名会话</p>
    </section>
  </main>
  ...
  <div id="dashboard-view" class="app-shell is-hidden">
    <header class="topbar">
      ...
      <div class="topbar-actions">
        ...
        <button id="refresh-button" class="icon-button" type="button" aria-label="刷新数据" title="刷新数据">↻</button>
        <button id="logout-button" class="button button-quiet" type="button">退出</button>
      </div>
    </header>
    ...
    <main class="dashboard">
      <section class="welcome-row">
        <div>
          ...
          <h1>访问概览</h1>
          <p class="subtle">不跟踪用户，只记录每个页面被打开的次数。</p>
        </div>
        <button id="open-snippet" class="button button-primary" type="button">获取计数代码</button>
      </section>
      ...
      <section class="content-card">
        <div class="card-heading">
          ...
          <div class="domain-total"><span>当前域名</span><strong id="domain-total">全部</strong></div>
        </div>
        ...
        <div class="table-wrap">
          ...
          <div id="empty-state" class="empty-state is-hidden">
            <span>✦</span><h3>还没有页面数据</h3><p>添加计数脚本并访问网站后，数据会出现在这里。</p>
          </div>
        </div>
        <footer class="pagination">
          <span id="page-summary">0 个页面</span>
          <div><button id="prev-page" class="icon-button" type="button" aria-label="上一页">←</button><span id="page-number">1 / 1</span><button id="next-page" class="icon-button" type="button" aria-label="下一页">→</button></div>
        </footer>
      </section>
    </main>
  </div>
  ...
  <dialog id="edit-dialog" class="modal">
    <form id="edit-form" method="dialog">
      <div class="modal-heading"><div><p class="eyebrow">EDIT COUNT</p><h2>修改访问量</h2></div><button class="icon-button" value="cancel" formmethod="dialog" aria-label="关闭">×</button></div>
      ...
      <div class="modal-actions"><button class="button button-quiet" value="cancel" formmethod="dialog">取消</button><button class="button button-primary" value="default" type="submit">保存修改</button></div>
    </form>
  </dialog>

  <dialog id="snippet-dialog" class="modal modal-wide">
    <form method="dialog">
      <div class="modal-heading"><div><p class="eyebrow">INSTALL</p><h2>添加到你的网站</h2></div><button class="icon-button" value="cancel" aria-label="关闭">×</button></div>
      <p class="subtle">将脚本放在页面的 <code>&lt;/body&gt;</code> 前。下面两个元素可按需放在任意位置。</p>
      <pre><code id="snippet-code"></code></pre>
      <div class="modal-actions"><button class="button button-quiet" value="cancel">完成</button><button id="copy-snippet" class="button button-primary" type="button">复制代码</button></div>
    </form>
  </dialog>
